...
UnigramTokenizer <-function(x) unlist(lapply(ngrams(words(x), 3), paste, collapse = " "), use.names = FALSE)
...
ggplot(hfp.df, aes(reorder(names,-high.freq), high.freq)) +
  ...
  ggtitle("Trigram Weight Plot - TF-IDF") + theme(axis.text.x=element_text(angle=45, hjust=1))
...
ggplot(head(usefulWeight,15), aes(reorder(word,-weight), weight)) +
  geom_bar(stat = "identity", fill = "blue") +
  xlab("Trigrams") + ylab("Weight") +
  ggtitle("Trigram Weight Plot - Useful Rating") + theme(axis.text.x=element_text(angle=45, hjust=1))
```
...
#Create TF-IDF
tfidffreq <- colSums(tf_idf)
high.tfidffreq=tail(sort(tfidffreq),n=200)
...
usefulWeight2 <- head(usefulWeight, 200)
