...
CLEANING THE TEXT
```{r}
corpus.ng <- tm_map(corpus,removeWords,c(stopwords("en"),"i","we","was","tacos"))
...
high.freq=tail(sort(tfidffreq),n=500)
hfp.df=as.data.frame(sort(high.freq))
hfp.df$names <- rownames(hfp.df) 

ggplot(hfp.df, aes(reorder(names,-high.freq), high.freq)) +
  geom_bar(stat="identity") + 
  xlab("Terms") + ylab("Value") +
  ggtitle("Term values - TFIDF") + theme(axis.text.x=element_text(angle=30, hjust=1))
...
wordcloud(hfp.df$names,hfp.df$`sort(high.freq)`,max.words=300,scale=c(1,.5),random.order = F, colors=pal)
```
...
wordcloud(usefulWeight$word,usefulWeight$weight,max.words=100,scale=c(1,1),random.order = F, colors=pal)
```
...
usefulWeight2 <- head(usefulWeight, 20)
